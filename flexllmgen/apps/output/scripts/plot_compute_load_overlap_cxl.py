...
def gen_plot(mha_load_latency, mha_compute_latency, ffn_load_latency,
    ...
    plt.savefig(common.plot_dir + \
                f"{model}_compute_load_overlap_cxl_bs{batch_size}_{stage}.pdf",
        bbox_inches="tight")
...
scenarios = ["O0", "O1", "O3", "O4", "O5"]
plot_scenarios = ["O0", "O3", "O4", "O5"]
scenario_dir = {
    "O0"   : "compressed/",
    "O1"   : "compressed/",
}
...
for batch_size in common.batch_sizes[model]:
        ...
        for line in open(common.output_dir + scenario_dir[scenario] + \
            ...
            if line.startswith(model):
                ...
                if "layer 0" in line or f"layer {last_layer}" in line:
                    ...
                    ###################################
                    if scenario == "O0" and f"layer {last_layer}" in line:
                        in_prefill = False
                    continue
    ...
    del prefill_mha_load_latency[copy_scenario]
    del prefill_mha_compute_latency[copy_scenario]
    del prefill_ffn_load_latency[copy_scenario]
    del prefill_ffn_compute_latency[copy_scenario]
    del decode_mha_load_latency[copy_scenario]
    del decode_mha_compute_latency[copy_scenario]
    del decode_ffn_load_latency[copy_scenario]
    del decode_ffn_compute_latency[copy_scenario]

    gen_plot(prefill_mha_load_latency, prefill_mha_compute_latency,
                prefill_ffn_load_latency, prefill_ffn_compute_latency,
                plot_scenarios, model, "prefill", batch_size)
    gen_plot(decode_mha_load_latency, decode_mha_compute_latency,
                decode_ffn_load_latency, decode_ffn_compute_latency,
                plot_scenarios, model, "decode", batch_size)