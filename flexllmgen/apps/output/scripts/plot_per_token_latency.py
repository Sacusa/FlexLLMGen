...
def gen_plot(latency, configs, model_size, batch_size):
    ...
    plt.bar(x, [np.mean(latency[config]) for config in configs],
            edgecolor="black", zorder=3.5)
    ...
    axis.set_ylim([0, 62])
    axis.yaxis.set_major_locator(plt.MultipleLocator(10))
    ...
    plt.savefig(common.plot_dir + \
        f"opt_{model_size}_batch_size_{batch_size}_per_token_latency.pdf",
        bbox_inches="tight")
...
for line in open(common.output_dir + \
    ...
    if line.startswith(f"opt-{model_size}"):
        ...
        if warmup_completed:
            latency[config].append(float(line.split(" ")[-2]))

for config in configs:
    print(config, np.mean(latency[config]))
